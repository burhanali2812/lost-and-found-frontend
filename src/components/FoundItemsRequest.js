...
function FoundItemsRequest({foundItems, setFoundItems}) {
    ...
    const getLostItems = async () => {
        try {
            const response = await fetch("http://localhost:5000/auth/get-foundItems", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
            ...
            const data = await response.json();
            const filterFoundItems = data.foundItems.filter((items) => items.request === "disapproved");
    ...
    const verifyFoundItems = async (id, userID) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/verifyFoundItems/${id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
            ...
            getLostItems();
            try {
                const response = await fetch("http://localhost:5000/auth/pushNotification", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
                ...
                showToast("success", "Notification Sent Successfully", 3000, "top-right");
    ...
    const handleSearchingNotification = async (selectedCity,selectedCategory, userId) => {
        let fetchedItems;
        try {
            const response = await fetch(`http://localhost:5000/auth/search-lost?city=${selectedCity}&category=${selectedCategory}`
                , {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
                    }
        ...
        const checkSubcategory = fetchedItems.filter(item => item.subCategory === selectedSubcategory);
        console.log("Filtered by subCategory:", checkSubcategory);

        const checkBrand = checkSubcategory.filter(item => item.brand === selectedBrand);
        ...
        if (checkBrand && checkBrand.length > 0) {
            try {
              checkBrand.forEach(async (matchedItem) => {
                const notificationResponse = await fetch("http://localhost:5000/auth/pushNotification", {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const handleApprove = async (checkLostItem) => {
        ...
        await handleSearchingNotification(checkLostItem.city, checkLostItem.category,  checkLostItem.userId); // then search and notify
      };
      ...
      const declineFoundRequest= async () => {
        try {
            const response = await fetch(`http://localhost:5000/auth/verifyFoundItems/${selectedFoundItems._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const openMessageModal = (foundItems) => {
        ...
        const messageModal = new window.bootstrap.Modal(document.getElementById('messageModal'));
        messageModal.show();
    };
    ...
    const openEditModal = async (foundItems) => {
        ...
        if (foundItems) {
            ...
            const profileImageUrl = newUser.profileImage ? `http://localhost:5000/${newUser.profileImage.replace(/\\/g, '/')}` : "";
            ...
            const imageArray = Array.isArray(foundItems.imageUrl) ? foundItems.imageUrl : [foundItems.imageUrl].filter(Boolean);
      ...
      return (
        ...
        <div>
        {Loading && (
            <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center bg-dark bg-opacity-75" style={{ zIndex: 1055 }}>
                    ...
                    {foundItems.length > 0 ? (
                        foundItems.map((lostItem, index) => {
                            return (
                                ...
                                >
                                    <td>{index + 1}</td>
                                    <td>{users.find((u) => u._id === lostItem.userId)?.name || "Unknown User"}</td>
                                    ...
                                    <td>
                                        <button className="btn btn-outline-success btn-sm mx-1" title="Approve" onClick={() => handleApprove(lostItem)}>
                                            <i className="fas fa-check"></i>
                                        </button>
                                        <button className="btn btn-outline-danger btn-sm mx-1" title="Decline" onClick={() => openMessageModal(lostItem)}>
                                            <i className="fas fa-times"></i>
                                        </button>
                                        <button className="btn btn-outline-primary btn-sm mx-1" title="Details" onClick={() => openEditModal(lostItem)}>
                        ...
                        <tr>
                            <td colSpan="7" className="text-center">No Verification Request Available</td>
                        </tr>
                    )}
                </tbody>
        ...
        <div>
            <div className="modal fade" id="verifyLostItem" tabIndex="-1" aria-labelledby="verifyLostItemLabel" aria-hidden="true">
                        ...
                        <div>
                            <div className="modal-header border-0">
                                <h5 className="modal-title" id="verifyLostItemLabel">
                                    <i className="fas fa-search me-2 text-success"></i>Verify Lost Item Request | Lost & Found
                                </h5>
                                ...
                                <form className="container-fluid p-0 px-md-3 text-dark">
                                    ...
                                    <div className="row">
                                        <div className="col-12 text-center mb-3">
                                            <span className='bg-light p-1 rounded fw-bold d-inline-flex align-items-center'>
                                                <span className='bg-white rounded-circle d-flex justify-content-center align-items-center me-2' style={{ width: '25px', height: '25px' }}>
                                    ...
                                    <div className="row align-items-start px-2 px-md-3 text-white mt-4">
                                        ...
                                        <div className="col-12 col-md">
                                            <div className="row">
                                                <div className="col-12 col-sm-6 mb-2"><b>Name:</b> {name || "Loading..."}</div>
                                                <div className="col-12 col-sm-6 mb-2"><b>CNIC No:</b> {cnic || "Loading..."}</div>
                                                <div className="col-12 col-sm-6 mb-2"><b>Contact:</b> {contact || "Loading..."}</div>
                                                <div className="col-12 col-sm-6 mb-2"><b>Email:</b> {email || "Loading..."}</div>
                                                <div className="col-12 mb-2"><b>Address:</b> {address || "Loading..."}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="row text-white px-2 px-md-3">
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Title:</b> {title || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Category:</b> {selectedCategory || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Sub Category:</b> {selectedSubcategory || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Brand:</b> {selectedBrand || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>City:</b> {selectedCity || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Location:</b> {location || "Not provided"}</div>
                                        <div className="col-12 col-md-6 col-lg-4 mb-2"><b>Date Lost:</b> {date ? new Date(date).toLocaleDateString() : "Not provided"}</div>
                                        <div className="col-12 mb-2"><b>Description:</b> {description || "Not provided"}</div>
                                    </div>
            ...
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="messageModalLabel">Enter Decline Reason</h5>
                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div className="modal-body">
                        <form>
                            <div className="mb-3">
                                <label htmlFor="rejectionReason" className="form-label"><strong>Write Rejection Reason</strong></label>
                                <textarea className='form-control' rows="4" id="rejectionReason" value={reason} onChange={(e) => setReason(e.target.value)}></textarea>
                            </div>
                        </form>
                    </div>
                    <div className="modal-footer">
                        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="button" className="btn btn-primary" onClick={declineFoundRequest}>Submit</button>
                    </div>
                </div>
            </div>
        </div>
