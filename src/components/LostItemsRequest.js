   ...
   //const { setSelectedBrands } = useContext(BrandContext); 
    ...
    const getLostItems = async () => {
        try {
            ...
            const filterLostItems = items.filter((item) => item.request === "disapproved");
    ...
    const verifyLostItem = async (id, userID) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/verifyLostItems/${id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const handleSearchingNotification = async (selectedCity,selectedCategory, userId,selectedSubcategorys,selectedBrands) => {
        
        let fetchedItems;
        try {
            const response = await fetch(`http://localhost:5000/auth/search-found?city=${selectedCity}&category=${selectedCategory}`
                , {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
                    }
        ...
        const checkSubcategory = fetchedItems.filter(item => item.subCategory === selectedSubcategorys);
        console.log("Filtered by subCategory:", checkSubcategory);

        const checkBrand = checkSubcategory.filter(item => item.brand === selectedBrands);
        ...
        if (checkBrand && checkBrand.length > 0) {
            ...
            for (const saveitem of checkBrand) {
                ...
                try {
                  const notificationResponse = await fetch("http://localhost:5000/auth/postSavedItems", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                      "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const handleApprove = async (checkLostItem) => {
        ...
        await handleSearchingNotification(checkLostItem.city, checkLostItem.category,  checkLostItem.userId, checkLostItem.subCategory, checkLostItem.brand); // then search and notify
      };
    const declineLostItem = async () => {
        console.log("selectedlost items", selectedLostItems)
        try {
            const response = await fetch(`http://localhost:5000/auth/verifyLostItems/${selectedLostItems._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            ...
                            <button type="button" className="btn btn-primary" onClick={declineLostItem}>Submit</button>
                        </div>
                    </div>
                </div>
            </div>
