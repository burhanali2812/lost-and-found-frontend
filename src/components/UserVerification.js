...
function UserVerification({ user, setUser }) {
    ...
    const getUser = async () => {
        try {
            ...
            console.log(data);
            const filterUsers = data.user.filter((user) => user.isVerified === "requested");
            setUser(filterUsers);
    ...
    const addNotification = async (userIds, types, reasonMessage = "") => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/auth/verifyUser/${userIds}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const openEditModal = (user) => {
        if (user) {
            ...
            const editModal = new window.bootstrap.Modal(document.getElementById("verifyUser"));
            editModal.show();
            ...
            const profileImageUrl = user.profileImage ? `http://localhost:5000/${user.profileImage.replace(/\\/g, '/')}` : "";
            setProfileImage(profileImageUrl);

            const frontCnicImageUrl = user.frontCnic ? `http://localhost:5000/${user.frontCnic.replace(/\\/g, '/')}` : "";
            setFrontSideCnic(frontCnicImageUrl);

            const backCnicImageUrl = user.backCnic ? `http://localhost:5000/${user.backCnic.replace(/\\/g, '/')}` : "";
    ...
    return (
        <div className='container'>
                        ...
                        user.map((users, index) => {
                            return (
                                ...
                                >
                                    ...
                                    <td>{users.address}</td>
                                    <td>
                                        <button className="btn btn-outline-success btn-sm mx-1" title="Approve" onClick={() => addNotification(users._id, "accepted")}>
                                            <i className="fas fa-check"></i>
                                        </button>
                                        <button className="btn btn-outline-danger btn-sm mx-1" title="Decline" onClick={() => openMessageModal(users)}>
                                            <i className="fas fa-times"></i>
                                        </button>
                                        <button className="btn btn-outline-primary btn-sm mx-1" title="Details" onClick={() => openEditModal(users)}>
            ...
            <div className="modal fade" id="verifyUser" tabIndex="-1" aria-labelledby="verifyUserLabel" aria-hidden="true">
                <div className="modal-dialog modal-lg">
                    <div className="modal-content bg-dark text-light rounded-4 shadow">
                        <div className="modal-header border-0">
                            <h5 className="modal-title" id="verifyUserLabel">
                                <i className="fas fa-user-check me-2 text-success"></i>Verify User | Lost & Found
                            </h5>
                            ...
                            <form className="container text-dark p-3 rounded">
                                <div className="d-flex flex-column flex-md-row align-items-start mt-4 px-3 text-white">
                                    ...
                                    <div className="flex-grow-1">
                                        <div className="mb-2"><b>Name:</b> {name || "Loading..."}</div>
                                        <div className="mb-2"><b>CNIC No:</b> {cnic || "Loading..."}</div>
                                        <div className="mb-2"><b>Contact:</b> {contact || "Loading..."}</div>
                                        <div className="mb-2"><b>Email:</b> {email || "Loading..."}</div>
                                        <div className="mb-2"><b>Address:</b> {address || "Loading..."}</div>
                                    </div>
                                </div>
                                ...
                                <div className='d-flex justify-content-end' style={{ marginTop: 20 }}>
                                    <button type='button' className='btn btn-success' data-bs-dismiss="modal" onClick={() => addNotification(selectedUser._id, "accepted")}>
                                        <i className="fas fa-check me-2"></i>Accept
                                    </button>
                                    <button type='button' className='btn btn-danger ms-3' onClick={() => openMessageModal(selectedUser)}>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <form>
                                <div className="mb-3">
                                    <label htmlFor="rejectionReason" className="form-label"><strong>Select Rejection Reason</strong></label>
                                    <select
                                        id="rejectionReason"
                        ...
                        <div className="modal-footer">
                            ...
                            <button type="button" className="btn btn-primary" onClick={() => addNotification(selectedUser._id, "declined")}>Submit</button>
                        </div>
                    </div>
                </div>
            </div>
