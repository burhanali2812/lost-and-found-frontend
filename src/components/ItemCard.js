...
export default function ItemCard({ item, savedItem, onDelete, onSave }) {
    ...
    const rawImages = Array.isArray(item?.imageUrl) ? item.imageUrl : [item?.imageUrl || ''];
        ...
        const fetchPhoneNumber = async () => {
            try {
                if (savedItem?.itemId) {
                    ...
                    const response1 = await fetch(`http://localhost:5000/auth/get-foundItemById/${savedItem.itemId}`, {
                        method: "GET",
                        headers: {
        ...
        <div className="col-md-4 mb-4">
            <div className="card">
                ...
                <div className="d-flex align-items-center mb-1 mx-4 mt-2">
                    ...
                    <div>
                        ...
                        <small className="text-muted">
                            {new Date(item?.dateFound).toLocaleString() || "Date not available"}
                        </small>
                    </div>
                ...
                <div className="card-body">
                    <ItemButtons savedItem={savedItem} onDelete={onDelete} onSave={onSave}  phoneNumber={phoneNumber} setPhoneNumber={setPhoneNumber} 
                    userName={userName} setUserName={setUserName} title={title} description={description} location={location} city={city}/>
                </div>
            </div>
        </div>
    );
